AutoHtml.add_filter(:vimeo_embed).with(width: 440, height: 248, show_title: false, show_byline: false, show_portrait: false, frameborder: 0) do |text, options|
  text.gsub(/(https?:\/\/)?player.vimeo\.com\/video\/([A-Za-z0-9._%-]*)((\?|#)\S+)?(\/.+)?/) do
    vimeo_id = $2
    width  = options[:width]
