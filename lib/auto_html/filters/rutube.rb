AutoHtml.add_filter(:rutube).with(:width => 640, :height => 480, :frameborder => 0) do |text, options|
  ...
  rutube_id = text[regex, 4]
  width = options[:width]
  height = options[:height]
  frameborder = options[:frameborder]
  src = "//rutube.ru/video/embed/#{rutube_id}"
  %{<iframe src="#{src}" frameborder="#{frameborder}" width="#{width}" height="#{height}" webkitAllowFullScreen mozallowfullscreen allowfullscreen></iframe>}
end
